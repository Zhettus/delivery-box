...
typedef struct {
    ...
    bool left;   // left turn  (the light does its own sweep while held on)
    bool right;  // right turn
} light_t;

void    light_init(void);        // configure GPIO, everything off
void    light_set(light_t s);    // apply a full state atomically
light_t light_get(void);         // read the current desired state
